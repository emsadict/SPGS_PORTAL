...
if (isset($_POST["submit"]) && !empty($_POST["pass"])) {
    ...
    if (searchRecord("admitted_2022", "regno", $regno) == 0) {
        $message = "You have not been given admission yet. You cannot create a Student account.";
    } elseif ($password != $c_password) {
        // Check if passwords match
        $message = "Password and Confirm Password do not match.";
    } else {
        // Check if account already exists
        $result = resultnew("SELECT * FROM spgs_acc_2021_2022 WHERE username='$regno'");
        if (mysqli_num_rows($result) > 0) {
            $message = "You have already created an account, Click <a href='portal_login.php'>Here </a>to login to your account";
        } else {
            // Create account
            $session = "2024/2025";
            $question = "PG STUDENT";
            $answer = "PG STUDENT";
            $create = array($regno, md5($password), $question, $answer, $session);

            if (input($create, "spgs_acc_2021_2022")) {
                $message = "Account successfully created. Click <a href='portal_login.php'>Here</a>to update your record";
            } else {
...
<head>
  ...
  <title>CREATE STUDENT ACCOUNT - POSTGRADUATE SCHOOL::University of Medical Sciences, Ondo</title>
  ...
  <main id="main">
    ...
    <section id="contact" class="contact">
      ...
      <div class="container " data-aos="fade-up">
        ...
        <div class="col-md-4 offset-md-4" data-aos="fade-up">
       ...
       <div class="col-lg-12 form-group">
         <div class="section-title ">
          <h4 class=" alert-primary " >NEW STUDENTS CREATE ACCOUNT</h4>
        </div>
		            <label for="regno">APPLICATION NO.:</label>
                <input type="text" name="regno" class="form-control" id="regno" placeholder="MATRIC/APPLICATION NO" required>
        </div>
      </div>
      <div class="row text-center">
        ...
        <div class="col-lg-12 form-group">
              ...
              <input type="password" name="cpass"  class="form-control" id="cpass" placeholder="RE-ENTER PASSWORD" required>
          </div>
         </div>
      <div class="text-center"><button type="submit" name="submit" onclick="return confirm('Kindly review your record, Are you sure you want to Submit?')">CREATE ACCOUNT</button>
      	<br><br>
              </div>
              <div class="text-center"><p style="font-display: block; align-self: buttom;">ALREADY CREATED AN ACCOUNT? <br><a href="create_acct.php">LOGIN HERE</a></p></div>
              </div>
